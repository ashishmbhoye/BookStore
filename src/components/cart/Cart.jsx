...
//   console.log("cartdata",cartContext.cartData)
  ...
  const placeOrder = async () => {
    if (authContext.user.id) {
      const userCart = await cartService.getCart(authContext.user.id);
      const userCartResult = userCart.data.result;
    //   console.log("cartvalue", userCart.data.result.length)
      if (userCartResult.length) {
        try {
          let cartIds = userCartResult.map((element) => element.id);
          const newOrder = {
            userId: authContext.user.id,
            cartIds,
          };
          const res = await cartService.placeOrder(newOrder);
          if (res) {
            cartContext.updateCart();
            navigate("/home");
            toast.success("Your Order is Placed successfully",{position:"bottom-right"});
          }
        } catch (error) {
  ...
  return (
    <div>
      <div className="cart-container">
        <Typography sx={{margin:5}} variant="h2">Cart page</Typography>
        ...
        <div className="btn-wrapper">
          <Button sx={{":hover": {backgroundColor: "#d11932"}}}  onClick={placeOrder}>
            Place order
          </Button>
        </div>
      </div>
